...
def nearest_neighbor(matrix): 
 ...
 nearest_city = np.argmin([matrix[current_city][i] for i in range(n) if i not in 
visited_cities]) 
...
def calculate_distance_matrix(points): 
 ...
 for i in range(n): 
 for j in range(n): 
 dist_matrix[i][j] = np.sqrt((points[i][0] - points[j][0]) ** 2 + (points[i][1] -
points[j][1]) ** 2) 
 return dist_matrix 
...
print("Error Approximation:", (approx_distance - optimal_distance) / 
optimal_distance)
